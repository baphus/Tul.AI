...
function AmountCounter({ value, reduced }: { value: number; reduced: boolean }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      if (!start) start = now;
      const duration = Math.min(1, (now - start) / 620);
  ...
  return <span className="t-num text-[2rem] leading-none" style={{ fontVariationSettings: '"wght" 540' }}>{formatPeso(Math.round(value * progress))}</span>;
}
      ...
      <div className={cn("absolute inset-0 [transform-style:preserve-3d]", reduced ? "transition-none" : "transition-transform duration-500 ease-out")} style={{ transform: flipped ? "rotateY(180deg)" : undefined }}>
        <article className={faceClass} style={{ background: "var(--tint-face)" }} aria-hidden={flipped}>
          ...
          <div className="mb-4 flex items-end justify-between border-y border-hairline py-3.5">
            <div><p className="t-micro text-ink-mute">Requirements matched</p><p className="t-display-md t-num mt-1">{result.percent === null ? "—" : `${result.percent}%`}</p></div>
          ...
          <p className="mb-4 flex flex-wrap items-baseline gap-2"><AmountCounter key={card.id} value={card.amount} reduced={reduced} /><span className="t-caption text-ink-mute">{card.amountNote}</span></p>
